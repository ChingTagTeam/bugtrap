...
export async function postReviewComments(
  ...
  for (const f of findings) {
    ...
    comments.push({
      ...
      body: `**[${f.severity}] ${f.type}** — ${Math.round(f.confidence * 100)}% confidence\n\n${f.message}\n\n*Detected by BugTrap ${f.agent} agent*`,
    });
  }

  const summaryBody = verdict.safe
    ? `✅ **BugTrap: Safe to merge**\n\n${verdict.summary}`
    : `⛔ **BugTrap: Blocked on ${verdict.blockedOn} critical finding${verdict.blockedOn !== 1 ? 's' : ''}**\n\n${verdict.summary}`;
...
export async function setCommitStatus(
  ...
  await octokit.repos.createCommitStatus({
    ...
    description: verdict.safe
      ? 'BugTrap: safe to merge'
      : `BugTrap: blocked on ${verdict.blockedOn} finding${verdict.blockedOn !== 1 ? 's' : ''}`,
    context: 'BugTrap / code-review',
  });
}
...
export async function createFixPR(
  ...
  if (ctx.files.length !== 1) {
    ...
    await octokit.issues.createComment({
      ...
      body: `## BugTrap Automated Fix\n\n${description}\n\n\`\`\`\n${fixedCode}\n\`\`\``,
    });
    return '';
  }

  const filename = ctx.files[0];
  const fixBranch = `bugtrap-fix-${ctx.pullNumber}-${Date.now()}`;
  ...
  await octokit.repos.createOrUpdateFileContents({
    ...
    repo: ctx.repo,
    path: filename,
    message: `fix: BugTrap automated fix for PR #${ctx.pullNumber}`,
  ...
  const { data: fixPr } = await octokit.pulls.create({
    owner: ctx.owner,
    repo: ctx.repo,
    title: `fix: BugTrap automated fixes for PR #${ctx.pullNumber}`,
    body: `Automated fixes generated by BugTrap.\n\n${description}`,
    head: fixBranch,
    base: ctx.baseBranch,
  });
