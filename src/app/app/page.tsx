...
export default function AppPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#1e1e1e', color: '#d4d4d4', fontFamily: 'Red Hat Display, sans-serif' }}>
      {/* Nav */}
      <nav style={{ borderBottom: '1px solid #3c3c3c', padding: '16px 32px', display: 'flex', alignItems: 'center', gap: '12px' }}>
        <Link href="/" style={{ display: 'flex', alignItems: 'center', gap: '10px', textDecoration: 'none' }}>
          <Image src="/Sidecode-logo.png" alt="Sidecode" width={28} height={28} />
          <span style={{ fontWeight: 700, fontSize: '16px', color: '#d4d4d4', letterSpacing: '0.04em' }}>SIDECODE</span>
        </Link>
      </nav>

      <main style={{ maxWidth: '900px', margin: '0 auto', padding: '48px 24px' }}>
        {/* Input mode tabs */}
        <div style={{ display: 'flex', gap: '4px', marginBottom: '24px', background: '#252526', borderRadius: '6px', padding: '4px', width: 'fit-content' }}>
          {(['paste', 'pr'] as InputMode[]).map((m) => (
            <button
              ...
              style={{
                padding: '8px 20px',
                background: inputMode === m ? '#3c3c3c' : 'transparent',
                border: 'none',
                borderRadius: '4px',
                color: inputMode === m ? '#d4d4d4' : '#9d9d9d',
        ...
        <section style={{ marginBottom: '40px' }}>
          {inputMode === 'paste' ? (
            <>
              <label style={{ display: 'block', fontSize: '11px', fontWeight: 700, letterSpacing: '0.12em', color: '#9d9d9d', marginBottom: '10px' }}>
                PASTE YOUR CODE
              </label>
              <textarea
                value={code}
                ...
                onFocus={(e) => { e.currentTarget.style.borderColor = '#5C8AF0'; }}
            ...
            <>
              ...
              <input
                ...
                onBlur={(e) => { e.currentTarget.style.borderColor = '#3c3c3c'; }}
              />
              <p style={{ margin: '8px 0 0', fontSize: '12px', color: '#9d9d9d' }}>
                Sidecode will fetch the diff, post inline review comments, set a commit status, and open a fix PR.
              </p>
            </>
          )}
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '12px' }}>
            <button
              ...
              style={{
                background: scanning || !canScan ? '#3c3c3c' : '#5C8AF0',
                color: scanning || !canScan ? '#9d9d9d' : '#1e1e1e',
                border: 'none',
                borderRadius: '4px',
                padding: '10px 24px',
                fontFamily: 'Red Hat Display, sans-serif',
        ...
        {anyAgentActive && (
          <section style={{ marginBottom: '40px' }}>
            <div style={{ fontSize: '11px', fontWeight: 700, letterSpacing: '0.12em', color: '#9d9d9d', marginBottom: '16px' }}>
              ANALYSIS PROGRESS
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              ...
              {coordinatorRunning && (
                <div style={{ display: 'flex', alignItems: 'center', gap: '16px', padding: '12px 16px', background: '#252526', borderRadius: '6px', border: '1px solid #3c3c3c' }}>
                  <div style={{ width: '90px', flexShrink: 0 }}>
                    <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '11px', fontWeight: 700, color: '#5C8AF0', letterSpacing: '0.08em' }}>COORD.</span>
                  </div>
                  <div style={{ flex: 1, height: '4px', background: '#3c3c3c', borderRadius: '2px', overflow: 'hidden' }}>
                    <div style={{ height: '100%', borderRadius: '2px', background: '#5C8AF0', width: '70%', opacity: 0.7, transition: 'width 0.6s ease' }} />
                  </div>
                  <div style={{ width: '120px', flexShrink: 0, textAlign: 'right', fontFamily: 'JetBrains Mono, monospace', fontSize: '11px', color: '#9d9d9d' }}>
                    reconciling...
                  </div>
                </div>
              )}
        ...
        {(patchLoading || patch) && (
          <section style={{ marginTop: '32px' }}>
            ...
            {patchLoading && !patch && (
              <div style={{ color: '#9d9d9d', fontFamily: 'JetBrains Mono, monospace', fontSize: '12px' }}>generating fix...</div>
            )}
            {patch && (
              <div style={{ background: '#252526', border: '1px solid #3c3c3c', borderRadius: '6px', overflow: 'hidden' }}>
                <div style={{ padding: '12px 16px', borderBottom: '1px solid #3c3c3c', fontSize: '13px', color: '#d4d4d4' }}>
                  {patch.description}
                </div>
                <pre style={{ margin: 0, padding: '16px', fontFamily: 'JetBrains Mono, monospace', fontSize: '12px', lineHeight: '1.6', color: '#d4d4d4', overflowX: 'auto', maxHeight: '400px' }}>
                  {patch.fixedCode}
                </pre>
              </div>
            )}
        ...
        {/* Error */}
        {error && (
          <div style={{ background: 'rgba(242,109,120,.07)', border: '1px solid #f26d78', borderRadius: '6px', padding: '16px', color: '#f26d78', fontFamily: 'JetBrains Mono, monospace', fontSize: '13px', marginTop: '24px' }}>
            ERROR: {error}
          </div>
        )}
      </main>
...
function AgentRow({ name, state }: { name: AgentName; state: AgentState }) {
  const color = state.status === 'error' ? '#9d9d9d' : AGENT_COLOR[name];
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: '16px', padding: '12px 16px', background: '#252526', borderRadius: '6px', border: '1px solid #3c3c3c' }}>
      ...
      <div style={{ flex: 1, height: '4px', background: '#3c3c3c', borderRadius: '2px', overflow: 'hidden' }}>
        <div style={{
          height: '100%',
          borderRadius: '2px',
          background: state.status === 'error' ? '#9d9d9d' : color,
          width: state.status === 'idle' ? '0%' : state.status === 'running' ? '60%' : '100%',
          transition: 'width 0.6s ease',
          opacity: state.status === 'running' ? 0.7 : 1,
        }} />
      </div>
      <div style={{ width: '160px', flexShrink: 0, textAlign: 'right', fontFamily: 'JetBrains Mono, monospace', fontSize: '11px', color: '#9d9d9d' }}>
        {statusLabel}
      </div>
    </div>
  );
...
function FindingRow({ finding }: { finding: RankedFinding }) {
  const color = SEVERITY_COLOR[finding.severity] ?? '#9d9d9d';
  const agentColor = AGENT_COLOR[finding.agent];
  const confidence = Math.round(finding.confidence * 100);

  return (
    <div style={{ background: '#252526', border: '1px solid #3c3c3c', borderRadius: '6px', padding: '14px 16px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '6px', flexWrap: 'wrap' }}>
        <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '11px', fontWeight: 700, color, letterSpacing: '0.08em' }}>
          [{finding.severity}]
        </span>
        <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '11px', color: '#9d9d9d' }}>
          {finding.type}
        </span>
        {finding.line && (
          <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '11px', color: '#9d9d9d' }}>
      ...
      <p style={{ margin: 0, fontSize: '13px', color: '#d4d4d4', lineHeight: '1.5' }}>
        {finding.message}
      </p>
    </div>
  );
...
function DisagreementRow({ d }: { d: Disagreement }) {
  return (
    <div style={{ background: '#252526', border: '1px solid #3c3c3c', borderRadius: '6px', padding: '14px 16px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '6px', flexWrap: 'wrap' }}>
        <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '11px', color: '#e5c07b', fontWeight: 700 }}>
          ⚡ DISAGREEMENT
        </span>
        <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '11px', color: '#9d9d9d' }}>
          {d.type}
      ...
      <div style={{ fontSize: '12px', color: '#9d9d9d', marginBottom: '4px', fontFamily: 'JetBrains Mono, monospace' }}>
        {d.agents.map((a) => `${a}: ${d.severities[a] ?? '?'}`).join(' · ')} → coordinator ruled{' '}
        <span style={{ color: SEVERITY_COLOR[d.coordinatorRuling] ?? '#d4d4d4', fontWeight: 700 }}>{d.coordinatorRuling}</span>
      </div>
      <p style={{ margin: 0, fontSize: '13px', color: '#d4d4d4', lineHeight: '1.5' }}>
        {d.reason}
      </p>
...
function VerdictGate({
  ...
  const accentColor = blocked ? '#f26d78' : '#5C8AF0';
  ...
  return (
    <section style={{ marginTop: '40px' }}>
      ...
      <div style={{ border: `1px solid ${accentColor}`, borderRadius: '8px', padding: '24px', background: blocked ? 'rgba(242,109,120,0.08)' : 'rgba(92,138,240,0.08)' }}>
        ...
        <div style={{ marginTop: '16px', display: 'flex', flexWrap: 'wrap', gap: '16px', fontFamily: 'JetBrains Mono, monospace', fontSize: '11px', color: '#9d9d9d' }}>
          ...
          {githubStatus.fixPr && (
            <a href={githubStatus.fixPr} target="_blank" rel="noreferrer" style={{ color: '#5C8AF0' }}>
              ✓ fix PR opened →
            </a>
          )}
        </div>
